...
class GLADEncoder_conv(nn.Module):
    """
    the GLAD encoder described in https://arxiv.org/abs/1805.09655.
    """
    ...
    def forward(self, x, x_len, slot, default_dropout=0.2):
        ...
        # local_h = run_rnn(local_rnn, x, x_len)
        # global_h = run_rnn(self.global_rnn, x, x_len)
        local_h = local_conv(x.permute(0, 2, 1)).permute(0, 2, 1)
...
class GLADEncoder_global_conv_conditioned_pos_v1(nn.Module):
    """
    the GLAD encoder described in https://arxiv.org/abs/1805.09655.
    """
...
class Model_elmo(nn.Module):
    ...
    def __init__(self, args, ontology, vocab):
        ...
        self.utt_encoder = self.encoder(1024, args.dhid, self.ontology.slots, dropout=args.dropout)
        self.act_encoder = self.encoder(1024, args.dhid, self.ontology.slots, dropout=args.dropout)
        self.ont_encoder = self.encoder(1024, args.dhid, self.ontology.slots, dropout=args.dropout)
        self.utt_scorer = nn.Linear(2 * args.dhid, 1)
        self.score_weight = nn.Parameter(torch.Tensor([0.5]))
